...
class MixedIntegrator(Integrator):
    ...
    def __init__(
            self,
            numeric_integrator,
            system_of_shapes: SystemOfShapes,
        ...
        :param numeric_solver_dict: Dictionary containing the pre-analyzed ODE numerical solver expressions and serialized CSE metadata generated by the ODE-toolbox analysis.
        ...
        self.numeric_solver_dict = numeric_solver_dict
        ...
        self._parameters = {
            k: _sympy_parse_real(
                v,
                global_dict=Shape._sympy_globals).n() if not _is_sympy_type(v) else v for k,
            v in self._parameters.items()}  # translates string values into mathematical symbols
        self._locals = {}
        for parameter_name, parameter_expr in self._parameters.items():
            self._locals[sympy.Symbol(
                parameter_name, real=True)] = parameter_expr
        self.random_seed = random_seed

        self.analytic_solver_dict = analytic_solver_dict
        if self.analytic_solver_dict is not None:
            if "parameters" not in self.analytic_solver_dict.keys():
        ...
        # obtain numerical solver, only for cse cases
        if self.numeric_solver_dict is None:
            # generate numerical solver from system of shapes
            runtime_numeric_solver = (
                self._system_of_shapes.generate_numeric_solver())
        else:  # numerical solver has been provided
            runtime_numeric_solver = copy.deepcopy(self.numeric_solver_dict)

        # check cse is present in the provided numerical solver
        is_cse_active = _has_cse(runtime_numeric_solver)
        if is_cse_active:
            # expand serialised cse metadata into oridinary mathematical
            # expressions
            runtime_numeric_solver = (
                expand_cse_solver(runtime_numeric_solver))

        # cse disabled, numeric_solver_dict from sys_of_shapes generated from
        # system of shapes
        self._update_expr = (
            runtime_numeric_solver["update_expressions"].copy())

        # convert strings back into sympy expressions for MixedIntegrator
        # compatibility
        for symbol, expression in list(self._update_expr.items()):
            if isinstance(expression, str):
                self._update_expr[symbol] = (_sympy_parse_real(
                    expression, global_dict=Shape._sympy_globals))
        ...
        self.all_variable_symbols = [
            sympy.Symbol(
                ...
                real=True) for sym in self.all_variable_symbols]

        for sym, expr in self._update_expr.items():
            try:
                self._update_expr_wrapped[sym] = sympy.utilities.autowrap.autowrap(expr.subs(self._locals),  # compiles with raw math symbols and injects params for c++ codegen
                ...
                raise ParametersIncompleteException(
                    "Integration not possible because numerical values were not specified for all parameters.")
        self.symbolic_jacobian_wrapped = np.empty(
            self.symbolic_jacobian_.shape, dtype=object)

        # compile symbolic jacobian useful for stiffness vs non-stiffness
        # checks
        for i in range(self.symbolic_jacobian_.shape[0]):
            for j in range(self.symbolic_jacobian_.shape[1]):
                self.symbolic_jacobian_wrapped[i, j] = sympy.utilities.autowrap.autowrap(self.symbolic_jacobian_[i, j].subs(
                    self._locals), args=self.all_variable_symbols, backend="cython", helpers=Shape._sympy_autowrap_helpers)
    ...
    def integrate_ode(
        ...
        assert all([isinstance(k, sympy.Symbol) for k in initial_values.keys(
        )]), 'Initial value dictionary keys should be of type sympy.Symbol'
        ...
        if self.analytic_solver_dict is not None:
            analytic_integrator_spike_times = {sym: st for sym, st in self.get_spike_times(
            ).items() if str(sym) in self.analytic_solver_dict["state_variables"]}
            self.analytic_integrator = AnalyticIntegrator(
                self.analytic_solver_dict, analytic_integrator_spike_times)
            analytic_integrator_initial_values = {sym: iv for sym, iv in initial_values.items(
            ) if sym in self.analytic_integrator.get_all_variable_symbols()}
            # initialises state variables with their correct values
            self.analytic_integrator.set_initial_values(
                analytic_integrator_initial_values)
        ...
        for sym in self._system_of_shapes.x_:
            if sym not in initial_values.keys():
                initial_values[sym] = float(
                    self._system_of_shapes.get_initial_value(
                        str(sym)).evalf(
                        subs={
                            sympy.Symbol(
                                sym,
                                real=True): v for sym,
                            v in self._parameters.items()}))

        upper_bound_crossed = False
        y = np.array([initial_values[sym]
                     for sym in self._system_of_shapes.x_])
        ...
        gsl_stepper = self.numeric_integrator(
            len(y), self.step, self.numerical_jacobian)  # init GSL numerical engine
        control = odeiv.control_y_new(
            gsl_stepper,
            self.integration_accuracy_abs,
            self.integration_accuracy_rel)
        ...
        try:
            ...
            while t < self.sim_time:
                if self.alias_spikes:
                    ...
                    # "queue" the next upcoming spike for the next iteration of the while loop
                    idx_next_spike += 1
                ...
                while t < t_target:
                    ...
                    try:
                        if self.analytic_integrator is not None:
                            self.analytic_integrator.disable_cache_update()

                        # evolve.apply parameters: start time, end time,
                        # initial step size, start vector
                        t, h_suggested, y = evolve.apply(
                            t, t_target_requested, h_requested, y)
                    except FloatingPointError:
                        msg = "Failure of numerical integrator (method: %s) at t=%.2f with requested timestep = %.2f (y = %s)" % (
                            gsl_stepper.name(), t, h_requested, y)
                    ...
                    if h_min < h_min_lower_bound:
                        estr = "Integration step below %.e (s=%.f). Please check your ODE." % (
                            h_min_lower_bound, h_min)
                    ...
                    for shape in self._shapes:
                        if shape.upper_bound is not None:
                            idx = [str(sym) for sym in list(
                                self._system_of_shapes.x_)].index(str(shape.symbol))
                            upper_bound_numeric = float(
                                shape.upper_bound.evalf(subs=self._locals))
                ...
                self._locals.update(
                    {sym: y[i] for i, sym in enumerate(self._system_of_shapes.x_)})
                ...
                if self.alias_spikes:
                    ...
                    while t_next_spike <= t:
                        ...
                        for sym in syms_next_spike:
                            if str(sym) in [str(sym_)
                                            for sym_ in self._system_of_shapes.x_]:
                                idx = [
                                    str(sym_) for sym_ in list(
                                        self._system_of_shapes.x_)].index(
                                    str(sym))
                                y[idx] += float(
                                    self._system_of_shapes.get_initial_value(sym).evalf(
                                        subs=self._locals))
                        idx_next_spike += 1
                        if idx_next_spike < len(all_spike_times):
                            t_next_spike = all_spike_times[idx_next_spike]
        ...
        if debug:
            ...
            if self._debug_plot_dir:
                self.integrator_debug_plot(
                    t_log, h_log, y_log, dir=self._debug_plot_dir)
    ...
    def integrator_debug_plot(self, t_log, h_log, y_log, dir):
        ...
        for i, sym in enumerate(analytic_syms):
            ax[i + y_log.shape[1]].plot(t_log,
                                        analytic_y_log[sym],
                                        label=str(sym),
                                        marker="o",
                                        color="chartreuse")

        ax[-1].semilogy(t_log[1:], h_log, linewidth=2,
                        color="grey", marker="o", alpha=.7)
        ...
        fn = os.path.join(dir, "mixed_integrator_[run=" + str(
            hash(self)) + "]_[run=" + str(self.numeric_integrator) + "].png")
    ...
    def numerical_jacobian(self, t, y, params):
        ...
        for row in range(0, dimension):
            for col in range(0, dimension):
                ...
                # dfdy[row, col] = float(self.symbolic_jacobian_[row,
                # col].evalf(subs=self._locals))      # non-wrapped version
    ...
    def step(self, t, y, params):
        ...
        # y holds the state of all the symbols in the numeric part of the
        # system; add those for the analytic part
        y = [self._locals[sym] for sym in self.all_variable_symbols]

        try:
            # return [
            # float(self._update_expr[str(sym)].evalf(subs=self._locals)) for
            # sym in self._system_of_shapes.x_ ]     # non-wrapped version
            _ret = [self._update_expr_wrapped[str(sym)](
                *y) for sym in self._system_of_shapes.x_]
        except Exception as e:
            logging.getLogger(__name__).error(
                "E==>", type(e).__name__ + ": " + str(e))
            logging.getLogger(__name__).error(
                "     Local parameters at time of failure:")
