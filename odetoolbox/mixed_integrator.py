...
class MixedIntegrator(Integrator):
    ...
    def __init__(self, numeric_integrator, system_of_shapes: SystemOfShapes, shapes: Iterable[Shape], analytic_solver_dict=None, parameters=None, spike_times=None, random_seed=123, max_step_size=np.inf, integration_accuracy_abs=1E-6, integration_accuracy_rel=1E-6, sim_time=1., alias_spikes=False, debug_plot_dir: Optional[str] = None, numeric_solver_dict=None):
        ...
        if self.numeric_solver_dict is None: 
            runtime_numeric_solver = (self._system_of_shapes.generate_numeric_solver()) # generate numerical solver from system of shapes
        else: # numerical solver has been provided 
            runtime_numeric_solver = copy.deepcopy(self.numeric_solver_dict)

        is_cse_active = _has_cse(runtime_numeric_solver) # check cse is present in the provided numerical solver
        if is_cse_active:
