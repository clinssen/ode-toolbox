...
class MixedIntegrator(Integrator):
    ...
    def __init__(self, numeric_integrator, system_of_shapes: SystemOfShapes, shapes: Iterable[Shape], analytic_solver_dict=None, parameters=None, spike_times=None, random_seed=123, max_step_size=np.inf, integration_accuracy_abs=1E-6, integration_accuracy_rel=1E-6, sim_time=1., alias_spikes=False, debug_plot_dir: Optional[str] = None, enable_cse=False, numeric_solver_dict=None):
        ...
        :param numeric_solver_dict: Dictionary containing the pre-analyzed ODE numerical solver expressions and serialized CSE metadata generated by the ODE-toolbox analysis. Required when enable_cse is True.
        ...
        self._parameters = {k: _sympy_parse_real(v, global_dict=Shape._sympy_globals).n() if not _is_sympy_type(v) else v for k, v in self._parameters.items()} # translates string values into mathematical symbols 
        self._locals = {}
        ...
        if self.numeric_solver_dict is None: 
            if self.enable_cse:
                raise ValueError("numeric solver must be supplied when enable_cse is activated otherwise its overwritten! ")
            runtime_numeric_solver = (self._system_of_shapes.generate_numeric_solver()) # generate numerical solver with cse metadata. 
        else: 
            runtime_numeric_solver = copy.deepcopy(self.numeric_solver_dict)
            
        if self.enable_cse:
            runtime_numeric_solver = (expand_cse_solver(runtime_numeric_solver)) # expand serialised cse metadata into oridinary mathematical expressions  

        # cse disabled, numeric_solver_dict from sys_of_shapes generated from system of shapes 
        self._update_expr = (runtime_numeric_solver["update_expressions"].copy())

        # convert strings back into sympy expressions for MixedIntegrator compatibility
        for symbol, expression in list(self._update_expr.items()):
            if isinstance(expression, str):
                self._update_expr[symbol] = (_sympy_parse_real(expression, global_dict=Shape._sympy_globals))
        ...
        for i in range(self.symbolic_jacobian_.shape[0]): # compile symbolic jacobian useful for stiffness vs non-stiffness checks 
    ...
    def integrate_ode(self, initial_values=None, h_min_lower_bound=5E-9, raise_errors=True, debug=False):
        ...
        if not self.analytic_solver_dict is None:
            ...
            self.analytic_integrator = AnalyticIntegrator(self.analytic_solver_dict, analytic_integrator_spike_times, enable_cse=self.enable_cse)
            ...
            self.analytic_integrator.set_initial_values(analytic_integrator_initial_values) # initialises state variables with their correct values 
        ...
        gsl_stepper = self.numeric_integrator(len(y), self.step, self.numerical_jacobian) # init GSL numerical engine 
