...
class TestSystemMatrixConstruction:
    ...
    def test_system_matrix_construction(self):
        indict = load_test_json("system_matrix_test.json")
    ...
    def test_lorenz_attractor(self):
        indict = load_test_json("lorenz_attractor.json")
        shapes, parameters = _from_json_to_shapes(indict)
    ...
    def test_morris_lecar(self):
        ...
        assert shape_sys.A_ == sympy.Matrix([[_sympy_parse_real("-500.0 * g_Ca / C_m - 1000.0 * g_L / C_m"), _sympy_parse_real("1000.0 * E_K * g_K / C_m")],
                                            [_sympy_parse_real("0"), _sympy_parse_real("0")]])

        V, W = sympy.symbols("V W", real=True)
        assert shape_sys.b_ == sympy.Matrix([[_sympy_parse_real("500.0 * E_Ca * g_Ca / C_m + 1000.0 * E_L * g_L / C_m + 1000.0 * I_ext / C_m")],
                                             [_sympy_parse_real("0")]])
        assert shape_sys.c_ == sympy.Matrix([[_sympy_parse_real("500.0 * E_Ca * g_Ca * tanh(V / 15 + 1 / 15) / C_m - 1000.0 * V * W * g_K / C_m - 500.0 * V * g_Ca * tanh(V / 15 + 1 / 15) / C_m")],
                                             [_sympy_parse_real("-200.0 * W * cosh(V / 60) + 100.0 * cosh(V / 60) * tanh(V / 30) + 100.0 * cosh(V / 60)")]])
